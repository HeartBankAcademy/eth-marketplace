...
recursive(paths.appBuild, (err, fileNames) => {
  let previousSizeMap = (fileNames || []).filter(fileName => /\.(js|css)$/.test(fileName)).reduce((memo, fileName) => {
...
function printFileSizes(stats, previousSizeMap) {
  let assets = stats
    ...
    .map(asset => {
      ...
      return {
        folder: path.join('build/app', path.dirname(asset.name)),
  ...
  let longestSizeLabelLength = Math.max.apply(null, assets.map(a => stripAnsi(a.sizeLabel).length))
...
function build(previousSizeMap) {
  ...
  webpack(config).run((err, stats) => {
    ...
    if (homepagePath && homepagePath.indexOf('.github.io/') !== -1) {
      ...
      console.log()
      console.log('The ' + chalk.cyan('build/app') + ' folder is ready to be deployed.')
      ...
      console.log('  ' + chalk.cyan('pushstate-server') + ' build/app')
